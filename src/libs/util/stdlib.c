...
void quicksort_internal(void* base, size_t num, size_t size, size_t left, size_t right, int (*compar)(const void*, const void*)) {
    if (left >= right) {
        return;
    }
    ...
    quicksort_internal(base, num, size, left, i - 1, compar);
    quicksort_internal(base, num, size, j + 1, right, compar);
}

void quicksort(void* base, size_t num, size_t size, int (*compar)(const void*, const void*)) {
    quicksort_internal(base, num, size, 0, num - 1, compar);
}