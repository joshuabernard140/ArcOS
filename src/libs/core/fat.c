...
bool Kernel_FAT_Initialize(Partition* part) {
    ...
    if (isFat32) {
        ...
        rootDirLba = Kernel_FAT_ClusterToLba( g_Data->BS.BootSector.EBR32.RootDirectoryCluster);
        rootDirSize = 0;
    } else {
    ...
    if (!Kernel_DISK_ReadSectors(part->disk, rootDirLba + part->partitionOffset, 1, g_Data->RootDirectory.Buffer, false)) {
...
FAT_File* Kernel_FAT_OpenEntry(Partition* part, FAT_DirectoryEntry* entry) {
    ...
    if (!Kernel_DISK_ReadSectors(part->disk, Kernel_FAT_ClusterToLba(fd->CurrentCluster) + part->partitionOffset, 1, fd->Buffer, false)) {
        printf("FAT: open entry failed - read error cluster=%u lba=%u\n", fd->CurrentCluster, Kernel_FAT_ClusterToLba(fd->CurrentCluster));
        for (int i = 0; i < 11; i++) {
...
uint32_t Kernel_FAT_NextCluster(Partition* part, uint32_t currentCluster) {    
    ...
    if (fatIndexSector < g_Data->FatCachePosition || fatIndexSector >= g_Data->FatCachePosition + FAT_CACHE_SIZE) {
        Kernel_DISK_ReadSectors(part->disk, g_Data->BS.BootSector.ReservedSectors + fatIndexSector, FAT_CACHE_SIZE, g_Data->FatCache, false);
    ...
    while (byteCount > 0) {
        ...
        if (leftInBuffer == take) {
            if (fd->Public.Handle == ROOT_DIRECTORY_HANDLE) {
                ...
                if (!Kernel_DISK_ReadSectors(part->disk, fd->CurrentCluster + part->partitionOffset, 1, fd->Buffer, false)) {
                ...
                if (++fd->CurrentSectorInCluster >= g_Data->BS.BootSector.SectorsPerCluster) {
                    fd->CurrentSectorInCluster = 0;
                    fd->CurrentCluster = Kernel_FAT_NextCluster(part, fd->CurrentCluster);
                }
                ...
                if (!Kernel_DISK_ReadSectors(part->disk, Kernel_FAT_ClusterToLba(fd->CurrentCluster) + fd->CurrentSectorInCluster + part->partitionOffset, 1, fd->Buffer, false)) {
                    printf("FAT: read error\r\n");
                    break;
                }
            }
...
bool Kernel_FAT_ReadEntry(Partition* part, FAT_File* file, FAT_DirectoryEntry* dirEntry) {
    return Kernel_FAT_Read(part, file, sizeof(FAT_DirectoryEntry), dirEntry) == sizeof(FAT_DirectoryEntry);
}
...
bool Kernel_FAT_FindFile(Partition* part, FAT_File* file, const char* name, FAT_DirectoryEntry* entryOut) {
    ...
    Kernel_FAT_GetShortName(name, shortName);

    while (Kernel_FAT_ReadEntry(part, file, &entry)) {
...
FAT_File* Kernel_FAT_Open(Partition* part, const char* path) {
    ...
    while (*path) {
        ...
        FAT_DirectoryEntry entry;
        if (Kernel_FAT_FindFile(part, current, name, &entry)) {
            Kernel_FAT_Close(current);
            ...
            current = Kernel_FAT_OpenEntry(part, &entry);
        } else {
            Kernel_FAT_Close(current);
...
void Kernel_FAT_ReadDirectory(Partition* part, const char* path) {
    FAT_File* fd = Kernel_FAT_Open(part, path);
    FAT_DirectoryEntry entry;
    int i = 0;
    printf("\n");
    while (Kernel_FAT_ReadEntry(part, fd, &entry) && i++ < 7) {
        printf("  ");
        for (int i = 0; i < 11; i++) {
            putc(entry.Name[i]); 
        } 
...
void Kernel_FAT_ReadFile(Partition* part, const char* path) {
    ...
    uint32_t read;
    FAT_File* fd = Kernel_FAT_Open(part, path);
    while ((read = Kernel_FAT_Read(part, fd, sizeof(buffer), buffer))) {
